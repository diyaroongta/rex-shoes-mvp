...
- THE BOTTOM NUMBER IS THE QUANTITY AS WRITTEN - transcribe it verbatim into "cartons" and do NOT convert it. Sometimes it is a carton count and sometimes it is a number of PAIRS; the app decides which from its size, and a wrong conversion here multiplies the order by the packing rate. Never turn pairs into cartons or cartons into pairs, never divide, never round.
...
SIZE RUN AND CLOSURE ARE SEPARATE. If a section is marked S or Small, set "group":"SMALL". If it is marked L, Large or Big, set "group":"LARGE". A free-standing L, or L beside a size, always means Large; NEVER turn that into Lace. Only the full word Lace sets "type":"LACE", and only the full word Velcro sets "type":"VELCRO". LEGACY ARTICLE-CODE EXCEPTION: when (L) or (V) is attached directly to an official catalogue article name such as REX GOLA (L), REX GOLA (V) or SMART BOY (L) WHITE, preserve the complete official article name in "category", BRACKETS AND ALL; that catalogue suffix is the closure - (L) is Lace, (V) is Velcro - and it selects a DIFFERENT article with a different BOM, so dropping it files the order against the wrong shoe. Do not convert an unattached L into that suffix. THERE IS NO "V" SIZE RUN: a bracketed (V) is always the Velcro closure and is never a run marker, so keep it even when the same sheet writes (L) elsewhere as a run. When one row of a sheet reads "Gola (V)" and another reads "Gola (L)", they are the Velcro and the Lace article of the same product and both brackets are closures. If the context is unclear, leave type empty for Match & Check instead of guessing. Do not carry a marker past a new one.
...
WORKED EXAMPLE - a sheet that reads: "1) Jindal Mahroli" / "Gola BLK  Large  8-over-2  9-over-3" / "Small  8-over-1  4-over-2  = 8 CTN" / "2) Paras Indore" / "Armour Lace  Large  1-over-1  2-over-1  3-over-1  = 3 CTN" / "3) Star Flw Manglore" / "Gola (V) BLK  1X3-over-1  4X5-over-1  = 2 CTN"
{"date":"","orders":[{"party":"Jindal Mahroli","category":"Gola","color":"Black","lines":[{"sizes":["8"],"cartons":2,"group":"LARGE"},{"sizes":["9"],"cartons":3,"group":"LARGE"},{"sizes":["8"],"cartons":1,"group":"SMALL"},{"sizes":["4"],"cartons":2,"group":"SMALL"}]},{"party":"Paras Indore","category":"Armour","color":"Black","lines":[{"sizes":["1"],"cartons":1,"group":"LARGE","type":"LACE"},{"sizes":["2"],"cartons":1,"group":"LARGE","type":"LACE"},{"sizes":["3"],"cartons":1,"group":"LARGE","type":"LACE"}]},{"party":"Star Flw Manglore","category":"Gola (V)","color":"Black","stated_cartons":2,"lines":[{"sizes":["1","3"],"cartons":1},{"sizes":["4","5"],"cartons":1}]}]}}
