...
**Job work and dispatch are NOT linked, and a remake is ASKED FOR.**
Dispatch completeness is ordered-versus-dispatched and knows nothing about job
work, so an order can read "complete" on the Dispatch Book while a job order
against it was closed short — both statements are true and about different
things.

`jobOrderBalance` counts what was ISSUED, never what came back, so issuing 500,
receiving 400 and writing 100 off short leaves `remaining` at zero. That number
is deliberately LEFT ALONE: quietly adding the lost pairs back would make a
hundred missing pairs disappear into an arithmetic. They surface instead as
`to_remake`, with `remake_allowance = remaining + to_remake` and `fully_issued`
false, because an order owing a remake is not finished. Raising the card is a
deliberate act — the operator ticks "remake these" and `api/dispatches.js` lifts
the cap by exactly that much for that request only, refusing `remake:true` on an
order with no shortage rather than treating it as a harmless no-op. A shortage
is recorded per JOB, never per size range, so on a remake the per-line and
per-size caps lift by the pool and the TOTAL is what bounds the card — splitting
it across combos would be inventing factory data.
...
| A success that reported itself as an error | Issuing a job order reduces the Order Book balance by exactly the pairs just issued — but the draft quantities stayed on screen, so the next render compared a SPENT draft against the reduced balance and printed "More than the unassigned balance was entered for 2X5, 6X8, 9X11" underneath a job order that had just been created successfully. The draft is cleared on success (the printable card keeps its own copy of the lines, taken at Preview, so the document is untouched), and the check is skipped once `card_no` exists. It still fires for a genuine over-entry on a draft. |
| A dropdown in storage order | The article picker listed articles in `Object.keys(articles)` order — the reference document's STORAGE order, whatever the uploads happened to write — so eleven Jacks were scattered between Glamour, Thunder and Rex Gola. It is grouped by FAMILY in `<optgroup>`s now, families A-Z so a name can be looked up, and within a family by CODE so JA001..JA011 read in order rather than alphabetically by colour. An uncoded article sorts by name at the END rather than being given an invented position. |
| A PI you could correct but not change | An uploaded PI offered only the numbers that were read — no way to add a size range, drop an article, or put a second one on, all of which a photographed slip allows. The controls had to write to `piCards`, because `sourceCards = piCards \|\| cards`: writing to `cards` would have been accepted on screen and silently ignored by the invoice, which is the same class of fault as an editable label the invoice never printed. A range added with every size at zero still prints nothing until a quantity is typed — a zero-pair line correctly emits no row. |
...
| A standing condition in the alarm channel | The no-BOM check was right and its DELIVERY was wrong: it pushed a sentence into `schedule_problems`, which renders as a red bar above EVERY screen. A capacity violation belongs there — it broke today's plan and someone can clear it. A BOM that has not been uploaded is a standing condition, still true next week, so the bar sat over the whole app permanently and taught people to scroll past the banner that means the plan really is broken. It is `data_gaps` now — structured (`article_code`, `pairs`, `orders`, `order_nos`), reported on the PROCUREMENT screen where the buyer is, because a buying list silently missing 10,015 pairs is that screen's problem. A test asserts it never reaches `schedule_problems`. |
