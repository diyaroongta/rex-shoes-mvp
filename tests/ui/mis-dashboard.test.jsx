...
const dispatches=[{order_no:"O-ON",dispatched:{A:80},dispatched_on:new Date().toISOString().slice(0,10),closes_order:false}];

describe("Executive MIS dashboard",()=>{
  it("shows management KPIs, dispatch completion and planned machine output",async()=>{
    const user=userEvent.setup();
    const refresh=vi.fn();
    render(<MISDashboard state={state} dispatches={dispatches} onRefresh={refresh}/>);
