...
```bash
npm test
npm run check   # regression tests followed by the production build
```

GitHub Actions runs `npm run check` on every pull request and every push to
`main`. A new feature should include a regression test for the old behaviour it
touches; merging is safe only when both the old contract and the new feature
pass together. The cross-feature suite specifically protects bulk workbook
parsing, PI-to-schedule restoration, and catalogue sole-routing edits.
