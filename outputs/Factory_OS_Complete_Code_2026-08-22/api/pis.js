...
async function reference(){
  ...
  }catch(_){ /* use the bundled reference */ }
  return SEED;
}
...
export default wrap(async (req,res)=>{
  ...
  if(req.method==="POST"){
    ...
    const existingRows=ids.length ? (await q(
      "select order_no, coalesce(pi->>'pi_no','') as pi_no from orders where order_no = any($1::text[])",[ids])).rows : [];
    const conflicts=existingRows.filter(r=>r.pi_no&&r.pi_no!==piNo);
    if(conflicts.length) return fail(res,409,`Order number already belongs to another PI: ${conflicts.map(r=>r.order_no).join(", ")}`);
    const existing=new Set(existingRows.map(r=>r.order_no));
    ...
    try{
      ...
      for(const order of restored.orders){
        if(existing.has(order.order_no)) continue;
    ...
    await syncPiMaster();
    return res.status(200).json({pi_no:piNo,restored:inserted,already_linked:[...existing]});
  }
