...
import { articleTypes, articleTypeCombos, comboSizesForArticle, pairsPerCarton, packingRuleSource } from "../shared/bridge.js";
...
export default function ArticleRulesTab({onChanged}){
  ...
  function chooseArticle(next){setArticle(next);setChosenType(articleTypes(next)[0]);setPackingEdits({});setMsg("");setErr("");}
  function chooseType(next){setChosenType(next);setPackingEdits({});setMsg("");setErr("");}
  ...
  async function savePacking(){
    ...
    try{
      for(const [combo,raw] of changed){
        ...
        if(!Number.isFinite(n)||n<1) throw new Error(`${combo}: pairs per carton must be 1 or more`);
      ...
      await api.patchReference({packing:groups});
      await reloadReference();
      setPackingEdits({});
      setMsg(article==="SPIKE"
        ? "Packing saved to ARMOUR, the source list SPIKE follows. Both articles now use the new values."
        : `Packing saved for ${article}. New and edited orders now use these carton quantities.`);
      onChanged&&onChanged();
  ...
  return <div>
    ...
    <div className="text-xs text-slate-600 bg-indigo-50 border border-indigo-200 rounded-xl px-3 py-2 mb-3">
      Edit <b>Pairs/carton</b> below and save. The change is stored in shared reference data and is used immediately by Match &amp; Check, bulk upload, order editing and new PIs.
    </div>
    ...
    <div className="flex gap-2 mt-3">
      <button disabled={busy||!Object.keys(packingEdits).length} onClick={savePacking}
      ...
      <button disabled={busy||!Object.keys(packingEdits).length} onClick={()=>setPackingEdits({})}
